import db

def obtener_materias(nombre = None, codigo = None):
    
    query = "FROM materias m WHERE 1=1"
    params = []

    if nombre:
        query += " AND m.nombre LIKE %s"
        params.append(f"%{nombre}%")
    if codigo:
        query += " AND m.codigo = %s"
        params.append(codigo)

    count_query = "SELECT COUNT(*) AS total " + query
    count_cursor = db.execute_query(count_query, tuple(params), un_solo_valor=True)

    total = count_cursor['total'] if count_cursor else 0

    select_query = "SELECT * " + query + " ORDER BY m.id"

    listado_materias = db.execute_query(select_query, tuple(params))
    return listado_materias, total

def crear_materia(nombre, codigo):
    query = """
        INSERT INTO materias (nombre, codigo)
        VALUES (%s, %s)
    """

    values = (
        nombre,
        codigo
    )

    nuevo_id = db.execute_query(query, values, modifica_db=True)
    return obtener_materia_por_id(nuevo_id)

def obtener_materia_por_id(materia_id):
    query = "SELECT * FROM materias WHERE id = %s"
    return db.execute_query(query, (materia_id,), un_solo_valor=True)

def eliminar_materia(materia_id):
    query = "DELETE FROM materias WHERE id = %s"

    db.execute_query(
        query,
        (materia_id,),
        modifica_db=True
    )

    return True

def remplazar_materia(materia_id, params):
    query = """
        UPDATE materias
        SET nombre = %s, codigo = %s
        WHERE id = %s
    """

    values = (
        params['nombre'],
        params['codigo'],
        materia_id
    )

    filas_afectadas = db.execute_query(query, values, modifica_db=True)
    if filas_afectadas > 0:
        return obtener_materia_por_id(materia_id)
    return None

def existe_codigo(codigo, excluir_id=None):
    if excluir_id is not None:
        query = """
            SELECT COUNT(*) as total
            FROM materias
            WHERE codigo = %s
              AND id != %s
        """
        params = (codigo.strip(), excluir_id)
    else:
        query = """
            SELECT COUNT(*) as total
            FROM materias
            WHERE codigo = %s
        """
        params = (codigo.strip(),)

    result = db.execute_query(
        query,
        params,
        un_solo_valor=True
    )

    return result["total"] > 0 if result else False


def obtener_cursos_de_materia(materia_id):
    query = """
        FROM cursos c
        WHERE c.materia_id = %s
    """

    params = (materia_id,)

    count_query = "SELECT COUNT(*) as total " + query

    count_cursos = db.execute_query(
        count_query,
        params,
        un_solo_valor=True
    )

    total = count_cursos["total"] if count_cursos else 0

    select_query = """
        SELECT
            c.id,
            c.materia_id,
            c.nombre,
            c.anio,
            c.cuatrimestre
    """ + query + " ORDER BY c.id ASC"

    cursos = db.execute_query(
        select_query,
        params
    )

    return cursos, total

def contar_cursos_de_materia(materia_id):
    query = """
        SELECT COUNT(*) as total
        FROM cursos
        WHERE materia_id = %s
    """

    result = db.execute_query(query, (materia_id,), un_solo_valor=True)
    return result["total"] if result else 0